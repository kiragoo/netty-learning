...
public class ImWorker {
    ...
    public void init() {
        ...
        createParentIfNeeded(ServerConstants.WORKER_PARENT_PATH);

        // 创建一个 ZNode 节点
        // 节点的 payload 为当前worker 实例

        try {
            byte[] payload = JsonUtil.pojo2Bytes(localNode);

            pathRegistered = client.create()
                    .creatingParentsIfNeeded()
                    .withMode(CreateMode.EPHEMERAL_SEQUENTIAL)
                    .forPath(ServerConstants.WORKER_PREFIX_PATH, payload);
    ...
    public long getIdByPath(String path) {
        ...
        int index = path.lastIndexOf(ServerConstants.WORKER_PREFIX_PATH);
        if (index >= 0) {
            index += ServerConstants.WORKER_PREFIX_PATH.length();
