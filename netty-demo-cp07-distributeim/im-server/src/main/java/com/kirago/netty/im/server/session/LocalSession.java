...
public class LocalSession implements ServerSession {
    ...
    public void bind() {
        ...
        channel.attr(ServerConstants.CHANNEL_NAME).set(JsonUtil.object2JsonString(user));
        isLogin = true;
    }

    public void unbind() {
        isLogin = false;
        SessionManager.inst().removeLocalSession(getSessionId());
        this.close();
    }
